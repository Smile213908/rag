...
【在线】问题 → bge-m3 稠密 + BM25 混合检索 Top-50 → bge-reranker 精排 Top-5 → 拒答第一层(阈值 0.7) → CRAG 质检(第二层,LLM 判断资料是否支撑) → Kimi(k3) 生成带引用答案
```
...
- 已知挂账:端到端延迟 P95 超标(k3 推理模型),M2 P0 模型路由治理,见验收报告。
